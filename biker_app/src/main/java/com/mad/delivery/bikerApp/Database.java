...
public final class Database {
    ...
    private Database() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            res.name = fakeProd.name.firstName();
            res.phoneNumber = fakeProd.phoneNumber.phoneNumber();
            res.description = fakeProd.lorem.paragraph(1);
            res.email = fakeProd.internet.email();
            res.city = fakeProd.address.city();
            res.road = fakeProd.address.streetName();
            res.houseNumber = fakeProd.address.buildingNumber();
            res.postCode = fakeProd.address.postcode();
            res.doorPhone = res.name;

            DateTimeFormatter dtf = DateTimeFormat.forPattern("dd/MM/yy hh:mm");
            DateTime from = new DateTime(2019, 3, 1, 19, 20, 30);
            DateTime to = DateTime.now();

            Order o = new Order(u,res, products, new DateTime(fakeProd.date.between(from.toDate(), to.toDate())));
